...
export default function SidebarHistory({ onSelect, onDelete }: SidebarHistoryProps) {
  const [history, setHistory] = useState<any[]>([]);

  // ✅ Load history safely
  useEffect(() => {
    try {
      const stored = JSON.parse(localStorage.getItem("arjunai_history") || "[]");
  ...
  const handleDelete = (item: any) => {
    const filtered = history.filter((h) => h.file !== item.file);
    setHistory(filtered);
    localStorage.setItem("arjunai_history", JSON.stringify(filtered));
    onDelete(item);
  };
  ...
  return (
    <div className="flex flex-col h-full bg-[#0d1528] border-r border-[#1d2840] w-64">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-2">
        {history.length === 0 ? (
          <p className="text-center text-xs text-gray-500 mt-4">No projects yet</p>
        ) : (
